...
lazy_static! {
    ...
    pub static ref BLOCKSTATS_COUNTER: GaugeVec = register_gauge_vec!("glusterfs_blockstats", "Glusterfs volume block stats", &["volume", "optype", "size_bucket"]).unwrap();
    pub static ref BLOCKSTATS_TOTAL_COUNTER: GaugeVec = register_gauge_vec!("glusterfs_blockstats", "Glusterfs volume total block stats", &["volume", "brick", "optype"]).unwrap();
